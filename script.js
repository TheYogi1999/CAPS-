/* v8: adds whole-word-only replacement option (Unicode-aware boundaries) */
localforage.config({name:'text-to-codes',storeName:'langdb'})
const DB_KEY='db-flat-v8'
const LAST_LANG_KEY='last-language-v8'
...
const toggleWhole=document.getElementById('wholeWordsToggle')
...
function digitsOnly(v){ const m=String(v||'').match(/\d+/g); return m? m.join('') : '' }
...
function parseAsSchemaA(arr){
  ...
  for(const r of arr){
    ...
    const number=digitsOnly(r.id ?? r.number ?? r.code ?? r.nummer)
...
function parseAsSchemaB(arr){
  ...
  const prio=['id','code','nummer','number']
  for(const want of prio){ const i=low.indexOf(want); if(i!==-1){ idCol=header[i]; break } }
  header.forEach((h,i)=>{ const l=low[i]; if(['description','beschreibung','desc','bezeichnung'].includes(l)) descCol=h })
  if(!idCol) return null
  const langCols=header.filter(h=>h!==idCol && h!==descCol)
  if(!langCols.length) return null
  const out=[]
  for(const r of arr){
    const idVal=digitsOnly(r[idCol]); if(!idVal) continue
    for(const lc of langCols){
      const phrase=norm(r[lc]); if(!phrase) continue
...
async function parseXlsx(file){
  ...
  const a=parseAsSchemaA(allRows); if(a) return {rows:a, schema:'A'}
  const b=parseAsSchemaB(allRows); if(b) return {rows:b, schema:'B'}
  return {rows:[], schema:'unknown'}
}

/* Build compiler:
   - caseSensitive: toggle
   - wholeWords: when true, wrap each escaped phrase with unicode-aware boundaries:
     (?<![\\p{L}\\p{N}_]) PHRASE (?![\\p{L}\\p{N}_])
   This prevents partial matches (z.B. "TAG" matcht nicht in "TAGES").
*/
function buildCompiler(language, rows, caseSensitive, wholeWords){
  const filtered=rows.filter(r=>r.language===language && r.phrase && r.number)
                     .sort((a,b)=>b.phrase.length-a.phrase.length)
  if(!filtered.length) return null
  const map=new Map(), parts=[]
  const boundaryL='(?<![\\p{L}\\p{N}_])'
  const boundaryR='(?![\\p{L}\\p{N}_])'
  for(const r of filtered){
    const key=caseSensitive? r.phrase : r.phrase.toUpperCase()
    if(!map.has(key)) map.set(key, r.number)
    const core=r.phrase.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')
    parts.push( wholeWords ? `${boundaryL}${core}${boundaryR}` : core )
  }
...
function rebuild(){
  compiler=buildCompiler(activeLang, rows, toggleCase.checked, toggleWhole.checked)
  refresh()
}
function fillLanguageSelect(){
  const langs=[...new Set(rows.map(r=>r.language))].sort((a,b)=>a.localeCompare(b))
  if(!langs.length){ langSel.disabled=true; langSel.innerHTML='<option value="">— keine Daten —</option>'; compiler=null; return }
...
async function handleFile(file){
  try{
    ...
    if(!parsed.length){ setStatus('Keine gültigen Zeilen gefunden.'); setBadge(false); return }
    rows=parsed
    ...
    console.error(err); setStatus('Fehler beim Lesen der XLSX: '+(err?.message||err)); setBadge(false)
  }
}
...
langSel.addEventListener('change', async e=>{ activeLang=e.target.value; window._lastLang=activeLang; await localforage.setItem(LAST_LANG_KEY, activeLang); rebuild() })
toggleCase.addEventListener('change', rebuild)
toggleWhole.addEventListener('change', rebuild)
...
clrDbBtn.addEventListener('click', async()=>{
  await localforage.removeItem(DB_KEY); setBadge(false); rows=[]; langSel.disabled=true; langSel.innerHTML='<option value=\"\">— keine Daten —</option>'; compiler=null; setStatus('Lokale Datenbank gelöscht.')
})

// copy buttons
document.body.addEventListener('click', (e)=>{
  const id=e.target.getAttribute?.('data-copy'); if(!id) return
  const el=document.getElementById(id); navigator.clipboard.writeText(el.textContent||''); showToast('Kopiert')
})
...
;(async function init(){
  try{
    ...
    if(Array.isArray(stored) && stored.length){
      rows=stored; fillLanguageSelect(); previewTable.innerHTML=toPreviewTable(rows); setStatus(`Lokal geladen: ${rows.length} Zeilen.`); setBadge(true)
    }else{ setBadge(false) }
